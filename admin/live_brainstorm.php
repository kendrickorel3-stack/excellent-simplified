...
// Basic admin check (best-effort): if users table has is_admin column, require it.
// Otherwise allow access (you can adjust to your auth).
...
$is_admin = false;
if ($CURRENT_USER_ID && columnExists($conn, 'users', 'is_admin')) {
    $s = $conn->prepare("SELECT is_admin FROM users WHERE id = ? LIMIT 1");
    $s->bind_param('i', $CURRENT_USER_ID);
    $s->execute();
    $tmp = $s->get_result()->fetch_assoc();
    $is_admin = (int)($tmp['is_admin'] ?? 0) === 1;
    $s->close();
} else {
    // fallback: allow if user present (adjust to your needs)
    $is_admin = $CURRENT_USER_ID ? true : false;
}
...
if ($action === 'participants') {
    ...
    $res = $conn->query("SELECT id, user_id, display_name, last_seen FROM chat_participants ORDER BY last_seen DESC LIMIT " . intval($limit));
    $out = [];
    while ($r = $res->fetch_assoc()) $out[] = $r;
    json_response(['success'=>true,'participants'=>$out]);
}

if ($action === 'kick_participant') {
    $id = intval($_POST['id'] ?? 0);
    if (!$id) json_response(['success'=>false,'error'=>'missing id']);
    $stmt = $conn->prepare("DELETE FROM chat_participants WHERE id = ? LIMIT 1");
  ...
  <div class="wrap">
    <div class="grid">
      ...
      <aside>
        ...
        <div class="card">
          <div class="card-header">
            <span class="card-title"><i class="fa fa-users"></i> Participants</span>
          </div>
          <div class="status-bar">
            <span class="pulse-dot" style="width:6px;height:6px"></span>
            <span>Auto-refreshing every 2.2s</span>
          </div>
  ...
  try {
    ...
    for (const p of j.participants){
      ...
      div.innerHTML = `<div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div class="participant-name">${escapeHtml(p.display_name || ('Guest-' + p.id))}</div>
            <div class="participant-time">${p.last_seen}</div>
          </div>
        </div>
        <button class="btn btn-danger" style="font-size:11px;padding:5px 10px" onclick="kick(${p.id})"><i class="fa fa-ban" style="font-size:10px"></i> Kick</button>`;
      wrap.appendChild(div);
    }
  } catch(e){ console.error(e); }
}
